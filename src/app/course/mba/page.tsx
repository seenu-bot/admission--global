...
function MBAExplorerPageContent() {
  ...
  // Fixed course ID for MBA course details
  const courseId = "AWerkslnEeOtQAixfhxA";
  ...
  // Helper function to detect MBA colleges from colleges collection
  const isMbaCollegeDoc = (data: any): boolean => {
    ...
    const mbaKeywords = ['mba', 'pgdm', 'pgp', 'master of business administration', 'business administration'];
    return mbaKeywords.some((keyword) => coursesOffered.includes(keyword));
  };
  ...
  useEffect(() => {
    ...
    const coursesUnsub = onSnapshot(
      ...
      (error) => {
        console.error("Error fetching MBA colleges from courses:", error);
        coursesLoaded = true;
        updateColleges();
      }
    );
    ...
    const collegesUnsub = onSnapshot(
      ...
      (snapshot) => {
        snapshot.forEach((doc) => {
          const data = doc.data();
          if (!isMbaCollegeDoc(data)) return;
      ...
      (error) => {
        console.error("Error fetching MBA colleges from colleges:", error);
        collegesLoaded = true;
        updateColleges();
      }
    );
  ...
  const handleSelectState = (state: string) => {
    ...
    router.replace(`/course/mba${params.toString() ? `?${params.toString()}` : ""}`, { scroll: true });
  };
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <button
              ...
              onClick={() => {
                ...
                setSelectedState("");
                setSelectedCountry("");
                router.replace("/course/mba", { scroll: true });
              }}
